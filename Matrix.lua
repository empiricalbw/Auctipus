Auctipus.Matrix = {}
Auctipus.Matrix.__index = Auctipus.Matrix

function Auctipus.Matrix:New(auctions)
    local m = {}
    setmetatable(m, self)
    m:Init(auctions)
    return m
...
function Auctipus.Matrix:Init(auctions)
    self.auctions = {}
    self.max_j    = 0
    self.max_opt  = 0
...
function Auctipus.Matrix:Optimize(max_j)
...
function Auctipus.Matrix:_GetElems(i, j, results)
    if i == 0 then
        return
...
function Auctipus.Matrix:GetElems(j)
